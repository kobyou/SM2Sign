...
Others:
****************************************************************/
static void BiToW(unsigned int Bi[], unsigned int W[])
{
	int i;
	unsigned int tmp;
...
Others:
*****************************************************************/
static void WToW1(unsigned int W[], unsigned int W1[])
{
	int i;
...
Others:
********************************************************************/
static void CF(unsigned int W[], unsigned int W1[], unsigned int V[])
{
	unsigned int SS1;
	unsigned int SS2;
	unsigned int TT1;
...
{
	unsigned char hash[32], hash1[32], hash2[32];

	char* msg_1 = (char*)"abc";
	char* msg_2 = (char*)"abcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcdabcd";
	char* chash1 = (char*)"66c7f0f462eeedd9d1f2d46bdc10e4e24167c4875cf2f7a2297da02b8f4ba8e0";
	char* chash2 = (char*)"debe9ff92275b8a138604889c18e5a4d6fdb70e5387e5765293dcba39c0c5732";

	hex2bytes(chash1, 64, hash1);
	hex2bytes(chash2, 64, hash2);
	...
	SM3((unsigned char*)msg_1, strlen(msg_1), hash);
	printf("sm3(msg_1): ");
	dump_byte(hash, sizeof(hash));

	if(0 != memcmp(hash, hash1, 32)) printf("sm3 hash test error\n\n");

	SM3((unsigned char*)msg_2, strlen(msg_2), hash);
	printf("sm3(msg_2): ");
	dump_byte(hash, sizeof(hash));

	if(0 != memcmp(hash, hash2, 32)) printf("sm3 hash test error\n\n");
	else printf("sm3 hash test pass\n\n");
}
#endif